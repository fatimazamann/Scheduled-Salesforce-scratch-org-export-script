...
/**
 * Choose which of `where.exe`'s hits Windows can actually execute.
 *
 * npm installs THREE shims side by side for a CLI like `sf`:
 *
 *     sf        a bash script, for Git Bash -- CreateProcess cannot run this
 *     sf.cmd    the Windows batch shim     -- runnable via cmd.exe
 *     sf.ps1    the PowerShell shim        -- not directly spawnable either
 *
 * `where sf` lists all of them, and the extensionless one often comes first.
 * Spawning that produces a bare `ENOENT`, which reads like "sf is not
 * installed" when in fact it is installed and on PATH. So the order is
 * explicit: a real .exe (no shell needed at all), then .cmd/.bat, and only
 * then a sibling lookup for an extensionless hit. Anything we cannot execute
 * yields null, so the caller reports a clear "not found" rather than ENOENT.
 *
 * Pure function of its inputs so it can be unit-tested off Windows.
 */
function pickWindowsExecutable(hits, exists = fs.existsSync) {
	if (!Array.isArray(hits) || hits.length === 0) return null;

	const exe = hits.find((h) => /\.exe$/i.test(h));
	if (exe) return exe;

	const batch = hits.find((h) => /\.(cmd|bat|com)$/i.test(h));
	if (batch) return batch;

	// Only unrunnable shims came back. npm puts the .cmd next to the bash
	// script, so look for the sibling that Windows can actually start.
	for (const hit of hits) {
		for (const ext of ['.cmd', '.exe', '.bat']) {
			if (exists(hit + ext)) return hit + ext;
		}
	}
	return null;
}
...
function resolveExecutable(name) {
	...
	return IS_WINDOWS ? pickWindowsExecutable(hits) : hits[0];
}
...
/**
 * Quote the COMMAND NAME for a cmd.exe line.
 *
 * This is deliberately different from how arguments are escaped, and the
 * difference matters. cmd.exe has to resolve the command name to a file on
 * disk, so it needs REAL grouping quotes when the path contains a space --
 * `"C:\Program Files\sf\bin\sf.cmd"`. Caret-escaping those quotes makes cmd
 * treat them as ordinary characters, and it then splits the name at the first
 * space and reports `'"C:\Program' is not recognized`.
 *
 * Arguments are the opposite case: they carry untrusted data, cmd does not
 * need to group them (the child re-parses the tail itself), so there every
 * metacharacter including the quotes is caret-escaped.
 *
 * The command name is a path we resolved ourselves, not user data. Windows
 * paths cannot contain `"`, and a `%` would still be expanded inside quotes,
 * so both are refused outright rather than escaped badly.
 */
function quoteCommandForCmd(exe) {
	if (/["%]/.test(exe)) {
		throw new Error(
			`Cannot safely invoke an executable whose path contains a quote or percent sign: ${exe}. ` +
				`Set "sfCliEntry" to the CLI's run.js instead, which bypasses cmd.exe entirely.`
		);
	}
	return `"${exe}"`;
}
...
function buildInvocation(exe, args) {
	if (IS_WINDOWS && /\.(cmd|bat)$/i.test(exe)) {
		...
		const line = [quoteCommandForCmd(exe), ...args.map(escapeForCmd)].join(' ');
...
/**
 * Find the Salesforce CLI's own JS entry point near a resolved shim.
 *
 * When this succeeds we can spawn `node run.js ...` with shell:false, which
 * removes cmd.exe from the picture completely -- no quoting, no metacharacter
 * escaping, and no trouble with spaces in paths. That is strictly safer and
 * faster than routing through the shim, so we prefer it whenever it is found.
 *
 * Layouts checked (npm global on Windows first, since that is the common case):
 *   <dir>/node_modules/@salesforce/cli/bin/run.js         %APPDATA%\npm\sf.cmd
 *   <dir>/../lib/node_modules/@salesforce/cli/bin/run.js  POSIX npm prefix
 *   <dir>/../node_modules/@salesforce/cli/bin/run.js
 *   <dir>/../client/bin/run.js                            standalone installer
 */
function findCliEntryNear(exePath, exists = fs.existsSync) {
	if (!exePath) return null;
	const dir = path.dirname(exePath);
	const candidates = [
		path.join(dir, 'node_modules', '@salesforce', 'cli', 'bin', 'run.js'),
		path.join(dir, '..', 'lib', 'node_modules', '@salesforce', 'cli', 'bin', 'run.js'),
		path.join(dir, '..', 'node_modules', '@salesforce', 'cli', 'bin', 'run.js'),
		path.join(dir, '..', 'client', 'bin', 'run.js'),
	];
