...
const DEFAULTS = {
	...
	//
	// `sf project retrieve start` refuses to run outside an SFDX project, so we
	// run it with its working directory set to projectDir, which holds nothing
	// but an sfdx-project.json. The retrieved files still land in the export
	// folder via --output-dir.
	metadata: {
		enabled: false,
		manifest: 'config/package.xml',
		projectDir: 'metadata-project',
...
function validate(cfg) {
	...
	if (cfg.metadata.enabled) {
		...
		if (typeof cfg.metadata.projectDir !== 'string' || !cfg.metadata.projectDir.trim()) {
			throw new ConfigError('metadata.projectDir must be a non-empty path.');
		}
...
function loadConfig(opts = {}) {
	...
	cfg.metadata = {
		...
		projectDir: resolvePath(cfg.metadata.projectDir),
	};
