...
const DEFAULTS = {
	...
	//
	// The retrieve runs in its own scratch SFDX project (workDir), not in the
	// export folder. The CLI constrains where it may work in several ways --
	// and a child's working directory cannot be renamed afterwards on Windows,
	// which the export folder always is. Keeping the two apart means those
	// rules stop depending on exportRoot, the layout, or the folder pattern.
	metadata: {
		enabled: false,
		manifest: 'config/package.xml',
		outputSubdir: 'metadata',
		// Scratch SFDX project the CLI retrieves into, deliberately OUTSIDE the
		// export tree: its working directory must never be a folder the run
		// renames. The result is moved into the backup afterwards.
		workDir: '.metadata-work',
...
function loadConfig(opts = {}) {
	...
	cfg.metadata = {
		...
		workDir: resolvePath(cfg.metadata.workDir),
	};
